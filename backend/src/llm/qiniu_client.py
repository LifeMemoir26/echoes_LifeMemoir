...
class QiniuAIClient:
    ...
    def chat(
        ...
        _retry_count: int = 0,  # 内部参数，记录重试次数
        **kwargs,
    ) -> LLMResponse:
        """
        发送聊天请求
        ...
        try:
            ...
            if e.response.status_code in [429, 500, 502, 503, 504]:
                max_retries = len(self.api_keys) * 3  # 最多重试所有key的三遍
                if _retry_count >= max_retries:
                    logger.error(f"API error {e.response.status_code}, max retries ({max_retries}) exceeded")
                    raise
                
                logger.warning(f"API error {e.response.status_code}, rotating to next key (retry {_retry_count + 1}/{max_retries})")
                self._rotate_key()
                
                # 等待一段时间后重试（指数退避，针对429错误增加基础延迟）
                import time
                base_delay = 3 if e.response.status_code == 429 else 1  # 429错误基础延迟3秒
                delay = min(base_delay * (2 ** _retry_count), 30)  # 最多30秒
                logger.info(f"等待 {delay} 秒后重试...")
                time.sleep(delay)
                
                # 递归重试
                return self.chat(messages, model, temperature, max_tokens, json_mode, _retry_count=_retry_count + 1, **kwargs)
        ...
        **kwargs,
        ...
        try:
            ...
            if e.response.status_code in [429, 500, 502, 503, 504]:
                max_retries = len(self.api_keys) * 3  # 最多重试所有key的三遍
                if _retry_count >= max_retries:
                    logger.error(f"Async API error {e.response.status_code}, max retries ({max_retries}) exceeded")
                    raise
                
                logger.warning(f"Async API error {e.response.status_code}, rotating to next key (retry {_retry_count + 1}/{max_retries})")
                self._rotate_key()
                
                # 异步等待一段时间后重试（指数退避，针对429错误增加基础延迟）
                base_delay = 3 if e.response.status_code == 429 else 1  # 429错误基础延迟3秒
                delay = min(base_delay * (2 ** _retry_count), 30)  # 最多30秒
                logger.info(f"等待 {delay} 秒后重试...")
                ...
                # 递归重试
                return await self.chat(messages, model, temperature, max_tokens, json_mode, _retry_count=_retry_count + 1, **kwargs)
            logger.error(f"HTTP error: {e.response.status_code} - {e.response.text}")
            raise
        except Exception as e:
