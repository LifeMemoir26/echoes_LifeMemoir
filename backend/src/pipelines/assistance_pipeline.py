...
from ..config import get_settings, InterviewAssistanceConfig
from ..llm.concurrency_manager import ConcurrencyManager
from ..database.vector_store import VectorStore
from ..database.sqlite_client import SQLiteClient
from ..interact.interview_assistant import (
    DialogueStorage,
    DialogueTurn,
    TextChunk,
    SupplementExtractor,
...
from ..interact.interview_assistant.dialogue_storage import UPDATE_EXPLORED
...
class AssistancePipeline:
    ...
    使用方法：
        ...
        # 或者手动创建（需要手动调用初始化）
        pipeline = AssistancePipeline(username="特朗普", concurrency_manager=concurrency_manager)
        await pipeline.initialize_pending_events()
    """
    ...
    def __init__(
        ...
        logger.info(
            f"AssistancePipeline initialized for user '{username}': "
    ...
    async def initialize_pending_events(self) -> int:
        ...
        注意：
        - 推荐使用 AssistancePipeline.create() 工厂方法，会自动调用此方法
        - 如果使用 __init__ 构造，需要手动调用此方法
        - 同一个 pipeline 实例可以多次调用此方法（会清空并重新初始化）
