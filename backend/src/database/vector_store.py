...
def test_vector_store():
    ...
    try:
        ...
        # 测试数据
        summaries = [
            "唐纳德·特朗普(川普)在纽约曼哈顿开始工作",
            "特朗普(老特)修复并重新开放了沃尔曼溜冰场(沃尔冰场)",
            "川普在中央公园附近购买了一套公寓"
        ]
        chunk_ids = [1, 1, 2]
        
        # 添加摘要
        print(f"\n添加 {len(summaries)} 个摘要...")
        store.add_summaries(summaries, chunk_ids)
        
        print(f"向量库中共有 {store.count()} 个文档")
        ...
        for i, (doc, meta, dist) in enumerate(zip(
            ...
            print(f"{i}. {doc}")
            print(f"   chunk_id: {meta['chunk_id']}, 距离: {dist:.4f}")
        
        # 测试根据chunk_id查询
        print(f"\n查询chunk_id=1的所有摘要：")
        chunk_summaries = store.get_by_chunk_id(1)
