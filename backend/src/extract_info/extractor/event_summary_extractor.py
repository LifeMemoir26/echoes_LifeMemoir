...
SUMMARY_EXTRACTION_PROMPT = """你是一个专业的对话摘要提取助手。请从以下对话文本chunk中提取对话概要。
...
class EventSummaryExtractor:
    ...
    def __init__(self, llm_client: BaseLLMClient, model: str = "claude-3.7-sonnet"):
        """
        初始化提取器
    ...
    async def extract_summaries(
        ...
        try:
            ...
            logger.error(f"提取事件摘要失败: {e}, response前500字符: {response[:500] if 'response' in locals() else 'N/A'}")
            return []
