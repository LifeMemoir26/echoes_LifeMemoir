...
def test_session_message_flush_and_sse_connected(monkeypatch):
    ...
    monkeypatch.setattr(interview_api, "create_interview_session", fake_create_interview_session)
    monkeypatch.setattr(interview_api, "add_dialogue", _noop)
    monkeypatch.setattr(interview_api, "flush_session_buffer", _noop)
    monkeypatch.setattr(interview_api, "get_interview_info", _dummy_info)
    ...
    with client.stream(
        ...
        for line in stream.iter_lines():
            if line:
                chunks.append(line)
            if any("event: connected" in c for c in chunks) and any("event: status" in c for c in chunks):
                break
...
def test_interview_sync_and_sse_error_semantics_are_consistent(monkeypatch):
    _clear_registry_sync()

    async def fake_create_interview_session(username: str):
        return DummyInterviewSession(thread_id=f"thread-{username}")

    async def fail_add_dialogue(*args, **kwargs):
        raise RuntimeError("boom")

    monkeypatch.setattr(interview_api, "create_interview_session", fake_create_interview_session)
    monkeypatch.setattr(interview_api, "add_dialogue", fail_add_dialogue)
    monkeypatch.setattr(interview_api, "get_interview_info", _dummy_info)
    ...
    sync_resp = client.post(
        f"/api/v1/session/{session_id}/message",
        json={"speaker": "user", "content": "trigger failure"},
    )
    assert sync_resp.status_code == 500
    sync_error = sync_resp.json()["detail"]["errors"][0]

    record = asyncio.run(registry.get(session_id))
    assert record is not None
    error_event = next(evt for evt in record.event_history if evt.event == "error")
    assert error_event.payload["error_code"] == sync_error["error_code"]
    assert error_event.payload["retryable"] == sync_error["retryable"]
    assert error_event.payload["trace_id"] == sync_error["trace_id"]
