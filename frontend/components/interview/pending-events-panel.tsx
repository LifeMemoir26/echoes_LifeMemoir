      ...
      {events.length === 0 ? (
        ...
        <div className="flex-1 overflow-y-auto space-y-1 pr-1">
          {events.map((event) => {
            ...
            return (
              <div key={event.id} className="rounded-lg border border-slate-200 bg-white overflow-hidden">
                <button
                  onClick={() => onToggle(event.id)}
                  className="w-full flex items-center gap-2 px-3 py-2 text-left cursor-pointer hover:bg-slate-50 transition-colors duration-150"
                >
                  {event.is_priority && (
                    <span className="shrink-0 w-2 h-2 rounded-full bg-rose-500" title="高优先级" />
                  )}
                  <span className="flex-1 text-sm text-slate-900 leading-snug">{event.summary}</span>
                  <span className="shrink-0 text-slate-500 text-xs">{isOpen ? "▲" : "▼"}</span>
                </button>
                {isOpen && (
                  <div className="px-3 pb-3 pt-1 border-t border-slate-200">
                    {event.explored_content ? (
                      ...
                      <p className="text-xs text-slate-500 italic">尚未探索</p>
                    )}
                  </div>
                )}
              </div>
